...
import providerTest from '../../testData/RealProviderTest.js';

import '../../styles/ImageGallery.css'; // CSS 파일 import

function PortfolioCopy(props){
    ...
    // const [reduxDate, setReduxDate] = useState(checkSelect.finalDate);
    // const [reduxArea, setReduxArea] = useState(checkSelect.finalArea);
    // const [reduxSubarea, setReduxSubarea] = useState(checkSelect.finalSubarea);
    ...
    // function setFinalWide(props){
    //     if(props.providerType === 1){
    //         dispatch(changePhotographer(props));
    //         dispatch(changeArea(props.providerArea));
    //         dispatch(changeSubarea(props.providerSubArea));
    //     }else if(props.providerType === 2){
    //         dispatch(changeHair(props));
    //         dispatch(changeArea(props.providerArea));
    //         dispatch(changeSubarea(props.providerSubArea));
    //     }else if(props.providerType === 3){
    //         dispatch(changeStudio(props));
    //         dispatch(changeArea(props.providerArea));
    //         dispatch(changeSubarea(props.providerSubArea));
    //     }
    // }
    ...
    return(
        <>
            {windowSize.width > 768 ?
                <div>
                    {click.studioClick === false ? <button className='studio-button' onClick={()=>{changeClick(3); setShow(showStudio);}}>스튜디오</button> :
                        <button className='studio-button-click' onClick={()=>{changeClick(3); setShow(showStudio);}}>스튜디오</button>}

                        {click.photoClick === false ? <button className='photo-button' onClick={()=>{changeClick(1); setShow(showPhoto);}}>사진작가</button> :
                            <button className='photo-button-click' onClick={()=>{changeClick(1); setShow(showPhoto);}}>사진작가</button>}
                        
                        {click.hmClick === false ? <button className='hm-button' onClick={()=>{changeClick(2); setShow(showHM);}}>헤,메</button> :
                            <button className='hm-button-click' onClick={()=>{changeClick(2); setShow(showHM);}}>헤,메</button>}
                </div>
                :
                null
            }
            <div className="gallery-container">
                {
                    show.map((value, index)=>{
                        return(
                            <img key={index} className='image-container' src={value.providerRepPhotoPath} onClick={()=>{
